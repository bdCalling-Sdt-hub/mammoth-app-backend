...
const createBillRecordToDB = async (billInfo: IBill) => {
  ...
  const report = await Report.findOne({ _id: billInfo.report });
  const stat = await Bill.checkReport(billInfo.report);
  if (!stat) {
    throw new ApiError(
      StatusCodes.UNAUTHORIZED,
      'Report is Not Eligable for bill'
    );
  }
...
const updateBillInfo = async (billInfo: IBill) => {
  const report = await Report.findOne({ _id: billInfo.report });
  const stat = await Bill.checkReport(billInfo.report);
  if (!stat || report?.status !== REPORT_STATUS.FINAL) {
    throw new ApiError(
      StatusCodes.UNAUTHORIZED,
      'Report is Not Eligable for bill'
    );
...
const getAllBillRecordsFromDB = async (query: Record<string, any>) => {
  ...
  const billsData = await bills.modelQuery
    .populate({
      path: 'report',
      select: [
        'patient',
  ...
  const filterArray = tempArray.filter(bill => {
    ...
    return (
      (!search ||
        ...
        bill?.billId?.includes(search) ||
        bill?.bill_date?.toString().includes(search)) &&
      (!query?.status ||
        bill?.isBilled === Boolean(query?.status === 'true' ? true : false))
    );
  });
...
const getBillRecordFromDB = async (bill_id: Types.ObjectId) => {
  ...
  const patient = await Patient.findOne({ _id: report.patient })
...
const changeBillStatusFromDB = async (
